...
//地面平整理后数据指针
pcl::PointCloud<pcl::PointXYZ>::Ptr
    cloud_non_ground(new pcl::PointCloud<pcl::PointXYZ>);
//半径滤波后数据指针
pcl::PointCloud<pcl::PointXYZ>::Ptr
...
//地面平整
 void PlaneFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in,
...
int main(int argc, char **argv) {
  ...
  private_nh.param("p_threshold", p_threshold, 0.01);
  ...
  //去除地面
  PlaneFilter(cloud_after_PassThrough, p_threshold);
  //对数据进行半径滤波
  RadiusOutlierFilter(cloud_non_ground, thre_radius, thres_point_count);
  //转换为栅格地图数据并发布
...
// 创建分割对象
    ...
    seg.setDistanceThreshold(p_threshold);         // 设置距离阈值

    seg.setInputCloud(cloud_in);
    seg.segment(*inliers, *coefficients);   // 进行分割

    // 提取非地面点云
    extract.setInputCloud(cloud_in);
    extract.setIndices(inliers);
    extract.setNegative(true);  // true 表示提取指定索引之外的点
    extract.filter(*cloud_non_ground);

    // 保存非地面点云
    pcl::io::savePCDFile<pcl::PointXYZ>(file_directory + "non_ground.pcd",
                                               *cloud_non_ground);
 }
...
void RadiusOutlierFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pcd_cloud0,
  ...
  for (auto& point : pcd_cloud0->points) {
        point.z = 0.0f;
    }
  //设置输入点云
